...
//      failure ΓÇö the analysis result is still returned).
//
...
const analyzeHeuristically = (parsedUrl) => {
  ...
  const noteParts = [];
  if (flags.includes('insecure_scheme')) noteParts.push('No HTTPS ΓÇö traffic is not encrypted.');
...
const enforceRateLimit = async (userId) => {
  ...
  try {
    ...
    if (error) return; // fail open ΓÇö don't block users on a count error
    ...
    // network/db hiccup ΓÇö don't block the scan
  }
};
...
export const runScan = async ({ url, scanType = 'product', userId }) => {
  ...
  // Row written to DB ΓÇö must match `public.scan_history` columns exactly.
  const row = {
    user_id: userId,
